 ...
 * Skips rects whose page canvas is missing or has zero width/height (not yet rendered).
 */
export function getSelectionImageDataUrl(
	...
	const validRects = rects.filter((r) => {
		const canvas = pageCanvases.get(r.pageIndex);
		return (
			canvas && canvas.width > 0 && canvas.height > 0 && r.w > 0 && r.h > 0
		);
	});
	if (validRects.length === 0) return null;

	const totalHeight = validRects.reduce((acc, r) => acc + r.h, 0);
	const maxWidth = Math.max(...validRects.map((r) => r.w));
	...
	for (const rect of validRects) {
		...
		if (!canvas) continue;
		ctx.drawImage(canvas, rect.x, rect.y, rect.w, rect.h, 0, y, rect.w, rect.h);
		y += rect.h;
	}
