...
function getBounds(
  ...
  for (const [pageIndex, canvas] of pageCanvases) {
    ...
    const contentLeft = canvasRect.left - containerRect.left + container.scrollLeft
...
export function SelectionOverlay() {
  ...
  const handleDrawingLayerMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const container = viewerContainerRef?.current
    if (!container || !bounds.containerRect || bounds.pageLayouts.size === 0) return
    const contentX = e.clientX - bounds.containerRect.left + container.scrollLeft
    const contentY = e.clientY - bounds.containerRect.top + container.scrollTop
    const hitPage = findPageAtContentPoint(bounds.pageLayouts, contentX, contentY)
